...
class YandexImage:
    ...
    def search(self, query: str, sizes: Size = 'large', ) -> list:
        f = open("proxy.txt")   # proxy path
        proxy = []
        lines = 0
        for i in f:
            lines += 1
            proxy.append(f.readline()[:-1])
        prox = proxy[random.randint(0, lines-1)]
        print(prox)
        request = requests.get('https://yandex.ru/images/search',
                               ...
                               headers=self.headers, proxies={"http": "http://" + prox}) #place here your proxy-type
